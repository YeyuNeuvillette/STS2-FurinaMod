...
public sealed class InviteSurintendanteChevalmarin : InviteCard, KnowledgeDemon.IChoosable
{
    public override bool CanBeGeneratedInCombat => false;

    public override int MaxUpgradeLevel => 0;

    public InviteSurintendanteChevalmarin()
    ...
    public async Task OnChosen()
    {
        ...
        var existingMinion = Owner.PlayerCombatState?.Pets.FirstOrDefault(p => p.Monster?.GetType() == typeof(SurintendanteChevalmarin));

        if (existingMinion != null)
        {
            await EnhanceExistingMinion(new ThrowingPlayerChoiceContext(), existingMinion, typeof(SurintendanteChevalmarin));
        }
        else
        {
            await MinionCmd.AddMinion<SurintendanteChevalmarin>(new ThrowingPlayerChoiceContext(), Owner, new MinionSummonOptions(
                MaxHp: 3m,
                Source: this,
    ...
    private async Task EnhanceExistingMinion(PlayerChoiceContext choiceContext, Creature existingMinion)
    {
        ...
        var pureWaterNote = existingMinion.GetPower<Actions.PureWaterNote>();
        if (pureWaterNote != null)
        {
            var enhancement = existingMinion.GetPower<Actions.PureWaterNoteEnhancement>();
            if (enhancement == null)
            {
                await PowerCmd.Apply<Actions.PureWaterNoteEnhancement>(choiceContext, existingMinion, 1m, Owner.Creature, this);
                enhancement = existingMinion.GetPower<Actions.PureWaterNoteEnhancement>();
            }
            
            if (enhancement != null)
            {
                enhancement.DynamicVars["DrawBonus"].BaseValue += 1m;
